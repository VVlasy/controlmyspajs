...
class ControlMySpa {
    ...
    async idm() {
        let req = await axios.get("https://iot.controlmyspa.com/idm/tokenEndpoint", {
            headers: {
                'Accept': "*/*",
    ...
    async login() {
        ...
        let req = await axios.post(this.tokenEndpoint, formData,
            {
                headers: {
                    'Accept': "*/*",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
    ...
    async getWhoAmI() {
        let req = await axios.get(this.whoami, {
            headers: {
                'Accept': "*/*",
                'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
                'Accept-Encoding': "br;q=1.0, gzip;q=0.9, deflate;q=0.8",
    ...
    async getSpa() {
        let req = await axios.get("https://iot.controlmyspa.com/mobile/spas/search/findByUsername?username=" + this.email, {
            headers: {
                'Accept': "*/*",
                'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
                'Accept-Encoding': "br;q=1.0, gzip;q=0.9, deflate;q=0.8",
        ...
        if (req.status == 200) {
            ...
            if (this.celsius) {
                this.currentSpa.currentState.desiredTemp = ((parseFloat(this.currentSpa.currentState.desiredTemp) - 32) * (5 / 9)).toFixed(1);
                this.currentSpa.currentState.targetDesiredTemp = ((parseFloat(this.currentSpa.currentState.targetDesiredTemp) - 32) * (5 / 9)).toFixed(1);
                this.currentSpa.currentState.currentTemp = ((parseFloat(this.currentSpa.currentState.currentTemp) - 32) * (5 / 9)).toFixed(1);
            }
    ...
    async setTemp(temp) {
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/setDesiredTemp", tempData,
            {
                headers: {
        ...
        if (req.status == 202) {
            let body = req.data;
            console.log(this.currentSpa.currentState.desiredTemp + (this.celsius ? ' C' : ' F') + ' => ' + temp + (this.celsius ? ' C' : ' F'));

            this.currentSpa.currentState.desiredTemp = body.values.DESIREDTEMP;
            if (this.celsius) {
                this.currentSpa.currentState.desiredTemp = ((parseFloat(body.values.DESIREDTEMP) - 32) * (5 / 9)).toFixed(1);
            }
        } else {
            // error getting idm
    ...
    async setTempRange(high) {
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/setTempRange", tempData,
            {
                headers: {
                    'Accept': "application/json",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
        ...
        if (req.status == 202) {
            ...
            let oldTemp = this.currentSpa.currentState.desiredTemp;

            if (this.waitForResult) {
                await this.sleep(5000);
                let newSpaData = await this.getSpa();

                console.log(oldTemp + (this.celsius ? ' C' : ' F') + ' => ' + newSpaData.currentState.desiredTemp + (this.celsius ? ' C' : ' F'));
    ...
    async setPanelLock(locked) {
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/setPanel", panelData,
            {
                headers: {
                    'Accept': "application/json",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
        ...
        if (req.status == 202) {
            ...
            let oldState = this.currentSpa.currentState.panelLock;
    ...
    async setJetState(deviceNumber, desiredState) { // numbers 0,1,2  || states: HIGH , OFF
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/setJetState", jetState,
            {
                headers: {
                    'Accept': "application/json",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
        ...
        if (req.status == 202) {
            ...
            let oldState = this.currentSpa.currentState.components.find((el, id) => {
            ...
            if (this.waitForResult) {
                ...
                let newState = newSpaData.currentState.components.find((el, id) => {
                    return el.componentType == "PUMP" && el.port == deviceNumber.toString()
                });
    ...
    async setBlowerState(deviceNumber, desiredState) { // numbers 0,1,2  || states: HIGH , OFF
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/setBlowerState", blowerState,
            {
                headers: {
                    'Accept': "application/json",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
    ...
    async setLightState(deviceNumber, desiredState) { // numbers 0,1,2  || states: HIGH , OFF
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/setLightState", lightState,
            {
                headers: {
                    'Accept': "application/json",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
    ...
    async toggleHeaterMode() {
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/toggleHeaterMode", toggle,
            {
                headers: {
                    'Accept': "application/json",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
        ...
        if (req.status == 202) {
            ...
            let oldState = this.currentSpa.currentState.heaterMode;

            if (this.waitForResult) {
                await this.sleep(5000);
                let newSpaData = await this.getSpa();

                let newState = newSpaData.currentState.heaterMode;
    ...
    async setFilterCycleIntervalSchedule(scheduleNumber, filterInterval, startTime) { // scheduleNumber 0,1  || filterInterval: this.scheduleFilterIntervalEnum || time: 24 hour format eg 20:00
        ...
        let req = await axios.post("https://iot.controlmyspa.com/mobile/control/" + this.currentSpa._id + "/setFilterCycleIntervalsSchedule", schedule,
            {
                headers: {
                    'Accept': "application/json",
                    'User-Agent': "ControlMySpa/3.0.2 (com.controlmyspa.qa; build:1; iOS 14.2.0) Alamofire/5.2.2",
        ...
        if (req.status == 202) {
            ...
            let oldState = this.currentSpa.currentState.components.find((el, id) => {
                return el.componentType == "FILTER" && el.port == deviceNumber.toString()
            });
